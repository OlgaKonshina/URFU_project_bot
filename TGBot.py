...
try:
    ...
    logging.critical(f"Конфигурационный файл не был загружен. Ошибка {e}", exc_info=True)
...
def start_message(message):
    try:
        ...
        logging.error(f'Приветственное сообщение не отправлено. Ошибка: {e}', exc_info=True)
...
def help_message(message):
    ...
    try:
        ...
        logging.error(f'Произошла ошибка, инфоонмационное сообщение не отправлено. {e}', exc_info=True)
...
def help_message(message):
    try:
        ...
        logging.error(f'Сообщение о выборе языка не было отправлено. Ошибка {e}', exc_info=True)
...
def get_media_messages(message):
    ...
    try:
        ...
        if message.content_type == 'voice':
            ...
            logging.error(f'Не удалось распознать сообщение, неверный формат {message.content_type}')
            return
        ...
        try:
            ...
            logging.info('Распознали аудео и перевели его в текст')
        except Exception as e:
           bot.send_message(message.from_user.id,"Что-то пошло не так c распознованием😣")
           os.remove(fname)
           os.remove(fname[:-4] + '.wav')
           logging.critical(f'Не удалось перевести аудио в текст. Ошибка: {e}', exc_info=True)
           return
